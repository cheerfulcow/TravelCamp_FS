...
//                .formLogin().loginPage("/index")
    //указываем на какой адрес будут отправляться данные с формы. Нам уже не нужно будет создавать метод в контроллере
    // и обрабатывать данные с формы. Мы задали url, который используется по умолчанию для обработки формы аутентификации
    // по средствам SS. SS будет ждать объект с формы аутентиф и затем сверять логин и пароль с данными в БД
